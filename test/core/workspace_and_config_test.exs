defmodule Core.WorkspaceAndConfigTest do
  ...
  test "workspace bootstrap can be implemented in after_create hook" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-hook-bootstrap-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace path is deterministic per issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-deterministic-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace reuses existing issue directory without deleting local changes" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-reuse-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace replaces stale non-directory paths" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-stale-path-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace rejects symlink escapes under the configured root" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-symlink-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-root-symlink-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove rejects the workspace root itself with a distinct error" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-root-remove-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace surfaces after_create hook failures" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-hook-failure-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace surfaces after_create hook timeouts" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-hook-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace creates an empty directory when no bootstrap hook is configured" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-workspace-empty-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace removes all workspaces for a closed issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-issue-workspace-cleanup-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace cleanup handles missing workspace root" do
    missing_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-missing-workspaces-#{System.unique_integer([:positive])}"
      )
  ...
  test "linear client normalizes blockers from inverse relations" do
    ...
    issue =
      Trackers.Linear.ResponseDecoder.normalize_issue(
        raw_issue,
  ...
  test "linear client paginates issue state fetches by id beyond one page" do
    ...
    assert query =~ "HortatorLinearIssuesById"
  ...
  test "workspace remove returns error information for missing directory" do
    random_path =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-missing-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace hooks support multiline YAML scripts and run at lifecycle boundaries" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-hooks-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-hooks-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails with large output" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-hooks-large-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook times out" do
    previous_timeout = Application.get_env(:hortator, :workspace_hook_timeout_ms)

    on_exit(fn ->
      if is_nil(previous_timeout) do
        Application.delete_env(:hortator, :workspace_hook_timeout_ms)
      else
        Application.put_env(:hortator, :workspace_hook_timeout_ms, previous_timeout)
      end
    end)

    Application.put_env(:hortator, :workspace_hook_timeout_ms, 10)

    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-workspace-hooks-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.workspace.root == Path.join(System.tmp_dir!(), "hortator_workspaces")
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      ...
      claude_mcp_config_path: "/etc/hortator/mcp.json",
      claude_effort: "high"
    )
    ...
    assert config.claude.mcp_config_path == "/etc/hortator/mcp.json"
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    workspace_root = Path.join("/tmp", "hortator-workspace-root")
  ...
  test "schema parse normalizes policy keys and env-backed fallbacks" do
    ...
    assert settings.workspace.root == Path.join(System.tmp_dir!(), "hortator_workspaces")
  ...
  test "remote workspace lifecycle uses ssh host aliases from worker config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "hortator-elixir-remote-workspace-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      workspace_path = "/remote/home/.hortator-remote-workspaces/MT-SSH-WS"
      ...
      case "$*" in
        *"__HORTATOR_WORKSPACE__"*)
          printf '%s\\t%s\\t%s\\n' '__HORTATOR_WORKSPACE__' '1' '#{workspace_path}'
          ;;
      esac
      ...
      assert trace =~ "__HORTATOR_WORKSPACE__"
      assert trace =~ "~/.hortator-remote-workspaces/MT-SSH-WS"
