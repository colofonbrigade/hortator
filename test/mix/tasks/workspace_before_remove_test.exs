defmodule Mix.Tasks.Workspace.BeforeRemoveTest do
  ...
  test "uses current branch for lookup when branch option is omitted" do
    with_fake_gh_and_git(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo test-owner/test-repo --head feature/workpad --state open --json number --jq .[].number"

        assert log =~ "pr close 101 --repo test-owner/test-repo"
        assert log =~ "pr close 102 --repo test-owner/test-repo"
      end
    )
  end
  ...
  test "closes open pull requests for the branch and tolerates close failures" do
    with_fake_gh(fn log_path ->
      ...
      assert log =~ "pr list --repo test-owner/test-repo --head feature/workpad --state open --json number --jq .[].number"
      assert log =~ "pr close 101 --repo test-owner/test-repo"
      assert log =~ "pr close 102 --repo test-owner/test-repo"
  ...
  test "formats close failures without command stderr output" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        log = File.read!(log_path)
        assert log =~ "pr list --repo test-owner/test-repo --head feature/no-output --state open --json number --jq .[].number"
        assert log =~ "pr close 102 --repo test-owner/test-repo"
      end
    )
  end
  ...
  test "no-ops when PR list fails for current branch" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo test-owner/test-repo --head feature/list-fails --state open --json number --jq .[].number"
    ...
    try do
      ...
      with_env(
        %{
          ...
          "REPO_CLONE_URL" => "git@github.com:test-owner/test-repo.git"
        },
        fn ->
          fun.(log_path)
        end
