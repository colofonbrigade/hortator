    ...
    quote do
      ...
      setup do
        workflow_root =
          Path.join(
            System.tmp_dir!(),
            "hortator-elixir-workflow-#{System.unique_integer([:positive])}"
          )
        ...
        on_exit(fn ->
          ...
          Application.delete_env(:hortator, :server_port_override)
          Application.delete_env(:hortator, :memory_tracker_issues)
    ...
    config =
      Keyword.merge(
        [
          ...
          workspace_root: Path.join(System.tmp_dir!(), "hortator_workspaces"),
          worker_ssh_hosts: [],
