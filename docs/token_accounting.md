...
This document explains how Codex reports token usage through the app-server protocol and how Hortator should account for it.
...
Hortator sees these events wrapped inside the app-server message stream.
...
For Hortator, `model_context_window` should be displayed or logged separately from spend.

## Recommended Accounting Strategy For Hortator
...
## What Hortator Should And Should Not Do
...
- Expect one thread to span multiple turns when Hortator reuses a live Codex thread.
...
## Practical Interpretation For Hortator Logs
...
That is a strong signal for Hortator:

- use absolute totals as the main accounting surface
- ignore last/delta values for totals

## Recommended Hortator Documentation Contract

If Hortator documents token reporting externally, the contract should be:

- Live token totals come from Codex thread-scoped cumulative usage.
- Incremental usage may also be emitted, but Hortator does not use it for totals.
